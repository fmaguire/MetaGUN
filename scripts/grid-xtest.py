...
[-xtest testing data for cross-validate] [parameters for svm-train]
[-path path of the package, default, ..]
dataset"""
...
global out_filename, png_filename, testing_file
...
def process_options(argv=sys.argv):
    ...
    global dataset_pathname, dataset_title, pass_through_string, testing_file
...
class LocalWorker(Worker):
    def run_one(self,c,g):
        cmdline = '%s -m 400 -c %s -g %s -v %s %s %s' % \
...
class SSHWorker(Worker):
    def __init__(self,name,job_queue,result_queue,host):
        ...
        self.cwd = os.getcwd()
    def run_one(self,c,g):
        cmdline = 'ssh -x %s "cd %s; %s -m 400 -c %s -g %s -w1 %s -w-1 %s -v %s %s %s"' % \
          (self.host,self.cwd,
           svmtrain_exe,c,g,wt,wf,fold,pass_through_string,dataset_pathname)
        result = Popen(cmdline,shell=True,stdout=PIPE).stdout
        for line in result.readlines():
            if find(line,"Cross") != -1:
                return float(split(line)[-1][0:-1])
...
class TelnetWorker(Worker):
    ...
    def run(self):
        ...
        tn.write("exit\n")               
    def run_one(self,c,g):
        cmdline = '%s -m 400 -c %s -g %s -w1 %s -w-1 %s -v %s %s %s' % \
          (svmtrain_exe,c,g,wt,wf,fold,pass_through_string,dataset_pathname)
...
def main():
    ...
    bin_file_path = os.sep.join([script_file_path, '..', 'bin'])
    
    global svmtrain_exe
    svmtrain_exe = os.sep.join([bin_file_path, 'svm-train'])
    print svmtrain_exe
    ...
    cmdline = '%s -m 400 -b %s -v %s -c %s -g %s %s %s' % \
          (svmtrain_exe, prob, fold, best_c,best_g, pass_through_string, testing_file)
    result = Popen(cmdline,shell=True,stdout=PIPE).stdout
    
    acc_file = testing_file + ".cross-acc"
    f = file( acc_file, 'w' )
    f.write("Best c=%s, g=%s, rate=%s\n" % (best_c, best_g, best_rate))
    for line in result.readlines():
        if find(line,"Cross") != -1:
            f.write( line )
        if find(line,"sn") != -1:

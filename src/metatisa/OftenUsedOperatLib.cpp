...
{
	int ss =(int) source.size(), ts = (int)target.size(); 
	std::pair<Ve_Pa_Str_I, int > hostInfo( Ve_Pa_Str_I(), ts) ;
...
{
	int ts = (int)target.size(); 
	int len = endPos - begPos + 1;
	std::pair<Ve_Pa_Str_I, int > hostInfo( Ve_Pa_Str_I(), ts) ;
	for( int i = begPos; i <= endPos - ts && source[i] != '\0'; ++i )
	{
		int mutNum = 0;
		for( int j = 0; j < ts; ++j )
		{
